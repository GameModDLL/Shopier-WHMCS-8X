...
Bu modül, özellikle WHMCS'in son sürümlerinde yaşanan _config fonksiyonu hatalarını çözmek ve ödeme deneyimini iframe içinde gömülü (embedded) olarak sunmak üzere optimize edilmiştir.
...
Gömülü Ödeme Deneyimi: Müşterileri ayrı bir sayfaya yönlendirmek yerine, ödeme formunu fatura sayfasının içinde, bir iframe alanında açar.
...
GitHub deposundaki tüm dosyaları indirin.

İndirdiğiniz dosyaların içinde bulunan shopier.php ve callback/shopier.php dosyalarını WHMCS kurulum dizininize kopyalayın:
...
Geri Dönüş URL (Callback URL): [SİZİN WHMCS ADRESİNİZ]/modules/gateways/callback/shopier.php

3. WHMCS Yönetici Ayarları
WHMCS Yönetici Paneli'ne giriş yapın.

Yapılandırma > Sistem Ayarları > Ödeme Ağ Geçitleri yolunu izleyin.

Tüm Ödeme Ağ Geçitleri sekmesine gidin ve listeden "Shopier ile Hızlı Ödeme" seçeneğini işaretleyip Değişiklikleri Kaydet'e tıklayın.

Gerekli alanlara API Key (Mağaza No) ve API Secret (Gizli Anahtar) bilgilerinizi girin.

Sayfanın alt kısmındaki Para Birimleri bölümünde satış yaptığınız para birimlerinin (Örn: TRY) seçili olduğundan emin olun ve ayarları kaydedin.
...
📜 Lisans
Bu proje WHMCS Eula Lisansı ile uyumludur. Detaylar için WHMCS Eula sayfasını kontrol edebilirsiniz.
...
This module has been optimized specifically to resolve common _config function errors encountered in recent WHMCS versions and to provide an embedded (iframe) payment experience.
...
Embedded Payment Experience: Opens the payment form inside an iframe on the invoice page, rather than redirecting the customer to a separate page.
...
Copy the shopier.php and callback/shopier.php files to your WHMCS installation directory:
...
Callback URL: [YOUR WHMCS URL]/modules/gateways/callback/shopier.php
...
Go to the All Payment Gateways tab and select "Shopier ile Hızlı Ödeme" (or the name you assigned) from the list, then click Save Changes.

Enter your API Key (Store No) and API Secret (Secret Key) in the required fields.

Ensure your sales currency (e.g., TRY) is checked under the Currencies section at the bottom of the page, and save the settings.
...
📜 License
This project is compliant with the WHMCS Eula License. For details, please check the WHMCS Eula page.